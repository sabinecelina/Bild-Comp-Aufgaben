...
def get_distance(point1, point2):
    difference_vector = np.array((point2[0] - point1[0], point2[1] - point1[1]))
    return np.linalg.norm(difference_vector)


def calculatecross_ratio(parallelLinePair_A, parallelLinePair_B, referenceObject, object, image):
    edit_image = image.copy()
    v_x = get_intersection_point(parallelLinePair_A[0][0], parallelLinePair_A[0][1], parallelLinePair_A[1][0],
                                 parallelLinePair_A[1][1])
    v_y = get_intersection_point(parallelLinePair_B[0][0], parallelLinePair_B[0][1], parallelLinePair_B[1][0],
                                 parallelLinePair_B[1][1])
    b = referenceObject[0]
    r = referenceObject[1]
    b_0 = object[0]
    t_0 = object[1]
